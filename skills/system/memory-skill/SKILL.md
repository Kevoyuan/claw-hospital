# 🧬 Memory科 - Memory Skill

> 适用症状：健忘、上下文Loss、忘记之前对话、不记得用户信息

## 症状识别

Agent 出现以下情况时，需要Memory科Diagnosis：
...
在 `AGENTS.md` Medium确保正确的 memory 加载：
...
```bash
# 一键自检
ls ~/workspace/memory/ && cat ~/workspace/MEMORY.md | head -20 && echo "✅ Memory恢复"
```
...
| 用户信息Loss | USER.md 被覆盖 | 重建 USER.md |
| 上下文Loss | 会话未持久化 | 检查存储 |
